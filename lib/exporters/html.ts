...
export function buildHtml(p: ExportPayload): string {
  ...
  const dramatis = p.includeDramatisPersonae && p.characters?.length
    ? `<section class="dramatis"><h2>${escapeXml(p.dramatisTitle || "Dramatis Personae")}</h2>${p.characters
        .map(
          (ch) =>
            `<div class="dp-entry"><h3>${escapeXml(ch.name)} <span class="dp-role">(${escapeXml(ch.role)})</span></h3>${
              ch.appearance ? `<p>${textToHtml(ch.appearance)}</p>` : ""
            }${ch.personality ? `<p>${textToHtml(ch.personality)}</p>` : ""}${
              ch.backstory ? `<p>${textToHtml(ch.backstory)}</p>` : ""
            }</div>`
        )
        .join("")}</section>`
    : "";
  ...
  const chapters = p.chapters
    .map(
      (c, i) =>
        `<section id="ch${i + 1}" class="chapter"><h1>${escapeXml(c.title)}</h1>${textToHtml(c.body)}</section>`
    )
    .join("\n");
...
<style>
  ...
  .dramatis .dp-entry { margin-bottom: 1.5em; }
  .dramatis .dp-role { font-weight: normal; font-style: italic; color: #555; }
