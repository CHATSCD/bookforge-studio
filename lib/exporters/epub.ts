...
export interface ExportChapter { title: string; body: string; }
...
<rootfiles><rootfile full-path="OEBPS/content.opf" media-type="application/oebps-package+xml"/></rootfiles>
  ...
  oebps.file(
    "toc.xhtml",
    xhtml("Table of Contents", `<nav epub:type="toc"><h1>${escapeXml(p.tocTitle || "Contents")}</h1><ol>${navItems}</ol></nav>`)
  );
